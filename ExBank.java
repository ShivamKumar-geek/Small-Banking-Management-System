			...
			{ 
				...
				case 2:
						...
						for(i=0;i<num;i++)
						{
							...
							{
								flag=1;
								System.out.println("Acc Type\t\ttName\t\tPhn No.\t\t\t Adhar Card No.\t\tBalance");
								if(acctype[i].equals("AccountClosed"))
									System.out.println("***********Account Closed*************");
								else
								...
								else
								{
									...
									else
									{
										...
										if((amt>Double.parseDouble(bal[i])) || (Double.parseDouble(bal[i])-amt<1000))
											System.out.println("Withdrawal cannot be performed!! Account balance not enough. Minimum account balance muct be 1000 rupees.");
										else
											withdraw(i,amt);
									}
								}
